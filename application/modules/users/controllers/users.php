...
class Users extends Public_Controller{
    ...
    function signup_teacher($id=false){
        if($_POST)
        {
            $captcha = $this->session->userdata('captcha');
            if(($_POST['captcha'] == $captcha) && !empty($captcha)){
                $user = new User($id);
                $_POST['user_type_id'] = 10;
                $_POST['level_id'] = 2;
                $_POST['m_status'] = "active";
                $user->from_array($_POST);
                $user->save();
  ...
  function register_teacher_form($id=false){
		$data['nursery'] = new Nursery($id);
		$this->template->build('register_teacher_form',$data);
	}
