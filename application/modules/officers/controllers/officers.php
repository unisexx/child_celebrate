...
class Officers extends Public_Controller
{
    ...
    function index()
    {
		...
		if(@$_GET['search']==1){ //ถ้ามีการกดปุ่มค้นหาให้แสดงข้อมูล
	        ...
	        if(user_login()->user_type_id == 1){ // admin เห็นทั้งหมด
	            $data['users']->where('user_type_id in (6,7,8,13)');
	        }elseif(user_login()->user_type_id == 6){ // เจ้าหน้าที่เขต เห็นเจ้าหน้าที่จังหวัด กับอำเภอ
	            $data['users']->where('(user_type_id in (7,8,13)) and area_province_id in (select area_province_id from area_provinces_detail where area_id = '.user_login()->area_id.')');
	        }elseif(user_login()->user_type_id == 7){ // เจ้าหน้าที่จังหวัด เห็นเจ้าหน้าที่อำเภอ
	            $data['users']->where('user_type_id in (8,13) and amphur_id in (select id from amphures where province_id = '.user_login()->province_id.')');
	        }
	...
	function get_district(){
		if($_POST){
			echo form_dropdown('district_id',get_option('id','district_name','districts','where province_id = '.$_POST['province_id'].' and amphur_id = '.$_POST['amphur_id'].' order by district_name asc'),@$_POST['district_id'],'','--- เลือกตำบล ---');
		}
	}
