...
$(document).ready(function(){
    ...
    $('select[name=province_id]').live('change',function(){
    	if($('select[name=user_type_id]').val() == 8 || $('select[name=user_type_id]').val() == 13){
    		$('.loading').show();
    ...
    $('select[name=amphur_id]').live('change',function(){
    	if($('select[name=user_type_id]').val() == 13){
    		$('.loading').show();
	    	$('select[name=district_id]').remove();
	    	$.post('officers/get_district',{
    ...
    <?php if(@$_GET['user_type_id'] == 13):?>
		...
		$.post('officers/get_district',{
    		province_id : '<?php echo @$_GET['province_id']?>',
    		amphur_id : '<?php echo @$_GET['amphur_id']?>',
    		district_id : '<?php echo @$_GET['district_id']?>'
    	},function(data){
			$('.district').html(data);
			$('.loading').hide();
		});
...
<form method="get" action="officers">
	...
	<?php if(user_login()->user_type_id == 1): //แอดมินเห็นทุกจังหวัด?>
		<?=form_dropdown('user_type_id',array('6'=>'เจ้าหน้าที่ประจำเขต','7'=>'เจ้าหน้าที่ประจำจังหวัด','8'=>'เจ้าหน้าที่ประจำอำเภอ','13'=>'เจ้าหน้าที่ประจำตำบล'),@$_GET['user_type_id'],'class="input-xlarge"','--- เลือกประเภทเจ้าหน้าที่ ---');?>
	<?php elseif(user_login()->user_type_id == 6):?>
		<?=form_dropdown('user_type_id',array('7'=>'เจ้าหน้าที่ประจำจังหวัด','8'=>'เจ้าหน้าที่ประจำอำเภอ','13'=>'เจ้าหน้าที่ประจำตำบล'),@$_GET['user_type_id'],'class="input-xlarge"','--- เลือกประเภทเจ้าหน้าที่ ---');?>
	<?php endif;?>
        ...
        <tr>
            ...
            <td>
                <?php
                    ...
                    }elseif($user->user_type_id == 13){
                        echo $user->district->district_name.' / '.$user->amphur->amphur_name.' / '.$user->amphur->province->name;
                    }
                ?>
            </td>
