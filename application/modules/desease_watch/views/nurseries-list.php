      ...
      <form id="formSearch" method="get" action="">
            <div style="padding:10px; border:1px solid #ccc; margin-bottom:10px;">
                  <div style='margin:10px 0;'>
                    <div class="searchLabel">ประเภท</div>
                    <select name="nursery_type" class="form-control">
                      <option value="">แสดงทั้งหมด</option>
                      <option value="1" <?php echo $select = @$_GET['nursery_type']=='1'? 'selected="selected"' : ''?>>ศูนย์เด็กเล็ก</option>
                      <option value="2" <?php echo $select = @$_GET['nursery_type']=='2'? 'selected="selected"' : ''?>>โรงเรียนอนุบาล</option>
                    </select>
                  </div>
		              <div style='margin:10px 0;'>
                        <div class="searchLabel">คำค้น</div>
                        <input name="name" type="text" value="<?php echo @$_GET['name']?>" placeholder="ชื่อศูนย์เด็กเล็ก/โรงเรียนอนุบาล" style="width:675px;"/>
                  </div>
                  <div style='margin:10px 0;'>
      ...
      <table class='table'>
            <thead>
                  <tr>
                        <th>ลำดับ</th>
                        <th>ประเภท</th>
                        <th>ชื่อศูนย์พัฒนาเด็กเล็ก/โรงเรียนอนุบาล</th>
                        <th>ที่อยู่</th>
                        <th>ปีที่เข้าร่วม</th>
                        <th>หัวหน้าศูนย์</th>
                        <th>ผลการประเมิน</th>
                  ...
                  <?php
                        if(empty($list)) {
                              ...
                              foreach($list as $item) {
                                    ...
                                    echo '<tr>';
                                          echo '<td>'.(++$no).'</td>';
                                          $nursery_type_name = $item->nursery_type == '2' ? 'โรงเรียนอนุบาล' : 'ศูนย์เด็กเล็ก';
                                          echo '<td>'.$nursery_type_name.'</td>';
