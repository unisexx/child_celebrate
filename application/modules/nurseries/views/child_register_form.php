<?php $title = $nursery_type == '1' ? 'ศูนย์เด็กเล็ก' : 'โรงเรียนอนุบาล';?>
<style>
...
$(document).ready(function(){
	...
	messages:
	{
		...
		name:
		{
			required: "กรุณากรอกชื่อ<?php echo $title;?>ค่ะ"
...
<ul class="breadcrumb">
  ...
  <li><a href="nurseries"><?php echo $title;?>ปลอดโรค</a> <span class="divider">/</span></li>
  <li><a href="nurseries/register">สมัครเข้าร่วมโครงการ<?php echo $title;?>ปลอดโรค</a> <span class="divider">/</span></li>
...
<form id="frmnursery" method="post" action="nurseries/register_save/<?=$nursery->id?>">
        ...
        <div id="data">
        <div style="font-size:14px; font-weight:700; padding-bottom:10px; color:#3C3">สมัครเข้าร่วมโครงการ<?php echo $title;?>ปลอดโรค</div>
    <!--search-->
    	<fieldset style="border:1px dashed #ccc; padding:10px; margin-bottom:10px;">
        <legend style="padding:0 5px; font-size:14px; font-weight:700; color:#666;">ข้อมูล<?php echo $title;?></legend>
        	<table class="table">
        		<tr>
                ...
                <?php }else if($nursery_type ==  1){?>
                <?=form_dropdown('nursery_type',array('1'=>'ศูนย์เด็กเล็ก'),@$nursery->nursery_type);?>
                <?php }else if($nursery_type == 2){?>
                <?=form_dropdown('nursery_type',array('2'=>'โรงเรียนอนุบาล'),@$nursery->nursery_type);?>
                <?php } ?>
        			</td>
        		</tr>
        		<tr>
        	    ...
        	    <tr>
        	      <th>ชื่อ<?php echo $title;?><strong> <span class="TxtRed">*</span></strong></th>
                 ...
                 <tr>
                    ...
                    <?php endif;?>
                    <?php if($nursery_type == 2):?>
                    <input type="radio" name="under" value="สพฐ" <?=($nursery->under == 'สพฐ')?"checked=checked":""?>> สพฐ
                    <input type="radio" name="under" value="อปท" <?=($nursery->under == 'อปท')?"checked=checked":""?>> เทศบาล,อบต
                    <input type="radio" name="under" value="กทม" <?=($nursery->under == 'กทม')?"checked=checked":""?>> กทม
                    <input type="radio" name="under" value="เอกชน" <?=($nursery->under == 'เอกชน')?"checked=checked":""?>> เอกชน
                 		<input type="radio" name="under" value="อื่นๆ" <?=($nursery->under == 'อื่นๆ')?"checked=checked":""?>> อื่นๆ
                 		<input type="input" name="under_other" value="<?=$nursery->under_other?>">
                    <?php endif;?>
        ...
        <legend style="padding:0 5px; font-size:14px; font-weight:700; color:#666;">
          หัวหน้า<?php echo $title;?>
        </legend>
        <table class="table">
   	    <tr>
        	      <th>คำนำหน้า<strong> <span class="TxtRed">*</span></strong></th>
