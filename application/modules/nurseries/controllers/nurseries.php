...
class Nurseries extends Public_Controller
{
	...
	{
		if(user_login()->user_type_id!=1 and user_login()->user_type_id!=6 and user_login()->user_type_id!=7 and user_login()->user_type_id!=8 and user_login()->user_type_id!=13 or user_login()->m_status != "active"){
	...
	function register_form($id=false){
		$data['nursery_type'] = @$_GET['nursery_type']!=''? $_GET['nursery_type'] : 1;
		$this->template->set_layout('blank');
		$data['nursery'] = new V_Nursery($id);
		$data['nursery_type'] = @$data['nursery']->nursery_type != '' ? $data['nursery']->nursery_type : $data['nursery_type'];
	...
	function check_name()
	{
		$nuchk = new Nursery();
		if(@$_GET['district_id']){
			$nuchk->where("district_id = ".$_GET['district_id']);
		}
		if(@$_GET['nursery_type']){
			$nuchk->where("nursery_type = ".$_GET['nursery_type']);
		}
