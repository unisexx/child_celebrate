...
function chk_center_status($id=false){
	$CI =& get_instance();
	$u = new User();
	$u->query("select id from users where (user_type_id = 9 or user_type_id = 11) and nursery_id = ".$id);
	if($u->exists())
	{
		return '<span style="color:#D14">ลงทะเบียนแล้ว</span> <a href="users/register_teacher_form/'.$id.'" class="btn btn-mini">ลงทะเบียนเจ้าหน้าที่ครู</a>';
	}
	else
	{
