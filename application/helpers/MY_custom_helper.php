...
function get_district_dropdown($amphur_id, $selected_value='', $show_all = false){
	...
	if($amphur_id>0){
		...
		@$ext_condition .= 'where id = '.$current_user->district_id;
   		echo form_dropdown('district_id',get_option('id','district_name','districts',$ext_condition.' order by district_name asc'),$current_user->district_id,'style="width:250px;"','--- เลือกตำบล ---');
	}else{ 
